import { Experience, Goal, MovementType } from "../types/workout";
 ...
 */
export const QUESTIONNAIRE_GOAL_MAP: Record<string, Goal> = {
...
export function resolveGoal(raw: unknown): Goal {
  ...
  const resolved = QUESTIONNAIRE_GOAL_MAP[value];
  if (!resolved) {
    throw new WorkoutEngineError(
      "invalid_goal",
      `Unsupported goal: ${JSON.stringify(raw)}`,
      `raw value did not match any of the ${Object.keys(QUESTIONNAIRE_GOAL_MAP).length} known goal strings`
    );
  }
  return resolved;
}

export const QUESTIONNAIRE_EXPERIENCE_MAP: Record<string, Experience> = {
...
export function resolveExperience(raw: unknown): Experience {
  const value = String(raw ?? "").toLowerCase().trim();
  const resolved = QUESTIONNAIRE_EXPERIENCE_MAP[value];
  if (!resolved) {
    throw new WorkoutEngineError(
      "invalid_experience",
      `Unsupported experience level: ${JSON.stringify(raw)}`
    );
  }
  return resolved;
}
...
export interface GoalStrategy {
  label: string;
  /** Rep range shown/used per experience tier. */
  reps: RepRange;
  /** Sets per exercise, per experience tier. */
  sets: { beginner: number; intermediate: number; advanced: number };
  restSeconds: number;
  /** Movement types this goal prefers, in priority order — used to bias exercise selection. */
  preferredMovementTypes: MovementType[];
  /** Whether a plyometric/explosive-movement exercise is ever allowed for this goal (still gated separately by experience — see exerciseSelectionService). */
  allowsPlyometric: boolean;
  /** How many of the week's days should be dedicated cardio/conditioning days, given N total days (function of frequency). */
  cardioDaysForFrequency: (days: number) => number;
 ...
 */
export const GOAL_STRATEGIES: Record<Goal, GoalStrategy> = {
  strength_training: {
...
export function getGoalStrategy(goal: Goal): GoalStrategy {
  return GOAL_STRATEGIES[goal];
}
