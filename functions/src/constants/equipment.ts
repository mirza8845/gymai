 ...
 */
const RAW_EQUIPMENT_MAP: Record<string, Equipment[]> = {
  everything: ["full_gym"],
 ...
 */
export function resolveEquipment(raw: unknown): Equipment[] {
  ...
  if (values.length === 0) {
    throw new WorkoutEngineError(
      "invalid_equipment",
      "No equipment selection found on the user's profile."
    );
  }
  ...
  if (unrecognized.length > 0) {
    ...
    logger.warn("equipment.unrecognized_value", { values: unrecognized });
  }
  ...
  if (resolved.size === 0) {
    ...
    logger.info("equipment.normalized_to_bodyweight", { rawValues: values });
 ...
 */
export function equipmentIsAvailable(required: Equipment, available: Equipment[]): boolean {
